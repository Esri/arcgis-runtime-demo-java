...
public class AddAndMoveGraphicsApp {
  ...
  // Default symbols
  private SimpleMarkerSymbol staticSymbol = new SimpleMarkerSymbol(Color.BLUE, 5, Style.CIRCLE);
  ...
  private GraphicsLayer staticGraphicsLayer;
  private JComboBox graphicsNumCombo;
  private HashMap<Integer, Point> latestStaticPoints = new HashMap<Integer, Point>();
  ...
  private JMap createMap() {
    ...
    // create a static graphics layer
    staticGraphicsLayer = new GraphicsLayer(RenderingMode.STATIC);
    // set a renderer for the graphics layer
    SimpleRenderer sourceRenderer = new SimpleRenderer(staticSymbol);
    staticGraphicsLayer.setRenderer(sourceRenderer);
    jMap.getLayers().add(staticGraphicsLayer);
  ...
  private void movePoints() {
    ...
    if (staticGraphicsLayer.getGraphicIDs()!=null) {
      for (int id : staticGraphicsLayer.getGraphicIDs()) {
        Point p1 = latestStaticPoints.get(Integer.valueOf(id));
        Point p2 = getPointForSmoothMove(p1.getX(), p1.getY(), id);
        staticGraphicsLayer.movePointGraphic(id, p2);
        latestStaticPoints.put(Integer.valueOf(id), p2);
      }
    }
  }
  ...
  private JToolBar createToolBar() {
    ...
    // static button
    JButton addStaticButton = new JButton("Add graphics (static)");
    addStaticButton.addActionListener(new ActionListener() {
      ...
      public void actionPerformed(ActionEvent e) {
        ...
        addStaticGraphicsBulk(numGraphicsToAdd);
      }
    });
